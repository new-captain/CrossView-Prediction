...
def TrainVIT_patch(XPath,Patch_channel=15,windowSize=25,encoded_dim=64,batch_size=128):
    import torch
    from DefinedModels import Dec_VIT, Enc_VIT
    from torchvision import transforms
    import numpy as np
    from tqdm import tqdm
    trans = transforms.Compose(transforms=[
    ...
    Enc_patch = Enc_VIT(channel=Patch_channel,output_dim=encoded_dim,windowSize=windowSize).cuda()
    Dec_patch = Dec_VIT(channel=Patch_channel,windowSize=windowSize,input_dim=encoded_dim).cuda()
    patch_data = AEDataset(XPath,trans)
    Patch_loader = torch.utils.data.DataLoader(dataset=patch_data, batch_size=batch_size, shuffle=True)
    optim_enc = torch.optim.Adam(Enc_patch.parameters(), lr=1e-3, weight_decay=0.0005)
    optim_dec=torch.optim.Adam(Dec_patch.parameters(), lr=1e-3, weight_decay=0.0005)
...
def SaveFeatures_VIT(XPath,Patch_channel=15,windowSize=25,encoded_dim=64,batch_size=128):
    import torch
    from DefinedModels import  Enc_VAE
    from torchvision import transforms
    import numpy as np
